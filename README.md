...
`reportType` 可取 `daily` 或 `weekly`，缺省保持 `daily`。日报与周报使用独立缓存和任务键，但共享浏览器并发 1 与模型并发 3。

旧版 `<shopId>-<date>.json` 缓存仍保留在服务器，但不再作为新 API 的命中来源。旧缓存没有保存页面接口的真实起止日期，在淘宝经营数据延迟入库时无法验证日期准确性；新版本首次查询会重新取数，并写入同时记录服务端请求周期和页面实际周期的新格式缓存。
